...
"""
from docpipe.refinement.edits import MAX_SHRINK, apply_edits
...
def test_a_clean_edit_is_applied():
    out, rep = apply_edits(TEXT, [{"find": "decarboni- sation",
...
def test_an_edit_that_is_not_there_is_refused():
    ...
    out, rep = apply_edits(TEXT, [{"find": "carbon capture",
                                   "replace": "CCS"}])
...
def test_an_ambiguous_edit_is_refused():
    ...
    out, rep = apply_edits(text, [{"find": "the value", "replace": "X"}])
    assert out == text
...
def test_an_edit_may_not_swallow_a_placeholder():
    ...
    out, rep = apply_edits(TEXT, [{"find": "in [p3_tbl0]", "replace": "above"}])
    assert out == TEXT
    assert "placeholder" in rep.rejected[0][1]


def test_an_edit_may_not_invent_a_placeholder():
    out, rep = apply_edits(TEXT, [{"find": "second half",
...
def test_a_wholesale_deletion_is_refused():
    ...
    text = "keep " * 100
    out, rep = apply_edits(text, [{"find": "keep " * 60, "replace": ""}])
    assert out == text
...
from docpipe.refinement.refine import _materialise_edits   # noqa: E402
...
def test_path_and_page_number_come_from_the_original_not_the_model():
    ...
    window = _window()
    out = _materialise_edits([{
        "index": 0, "_action": "keep", "title": "Wärmebedarf",
        "captions": {"p2_tbl0": "Energieträger"},
        "edits": [{"find": "Wärme- versorgung", "replace": "Wärmeversorgung"}],
    }], window)
...
def test_a_refused_edit_leaves_that_text_alone():
    ...
    out = _materialise_edits([{
        ...
        "edits": [{"find": "text that was never there", "replace": "x"}],
    }], window)
...
def test_a_reply_naming_a_section_outside_the_window_is_dropped():
    ...
    out = _materialise_edits([{"index": 7, "_action": "keep", "edits": []}],
                             _window())
    assert out == []
