...
def test_no_question_tells_the_model_where_it_may_quote_from(spec):
    """The code checks a quote for two things: it stands in a shown passage
    and it carries the answer. A question that restricts where the passage
    may stand has the model refuse readings no check refuses."""
    for parameter in spec.parameters:
        for name, axis in (parameter.axes or {}).items():
            text = axis.question or ""
