...
def parameter_states(spec, tuples: list, refusals: list, *,
    ...
    """
    ran = bool(harvested)
    failed = [r for r in refusals
              if (r.get("claim") or {}).get("_harvest_failed")]
    owners = {tuple(r["owner"]) for r in failed if r.get("owner")}
    anonymous = any(not r.get("owner") for r in failed)

    def cut_for(uri) -> bool:
        if not ran:
            return False
        if answered == 0:
            return True
        if not failed:
            return False
        own = (sources_of or {}).get(uri)
        if own is None or anonymous:
            return True
        return bool(owners & {tuple(k) for k in own})
