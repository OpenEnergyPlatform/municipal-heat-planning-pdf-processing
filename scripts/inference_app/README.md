...
The LLM endpoint/model default to the configured endpoint and the API key is read from a `.env`
(`LLM_API_KEY=...`). `LLM_STUB_MODE=1` exercises retrieval without calling the endpoint.
