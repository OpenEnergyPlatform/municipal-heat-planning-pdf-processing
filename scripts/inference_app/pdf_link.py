...
def best_quote_rects(pdf_path, page_number: int, quote: str,
    ...
    """
    Precise highlight rects for `quote` on a (1-based) PDF page, one box per text
    LINE the matched span touches, capped at `max_lines`.
    ...
    missing, the file/page is unavailable, or the best match scores under
    `min_score`; the caller then falls back to the `&search=` phrase.
    """
    try:
        import fitz
        from rapidfuzz import fuzz
