...
(`docpipe/extraction/schema.py:212-280`). The state is one of seven fixed
...
state is `read` or `derived` (`schema.py:283-297`). It is a finding about
...
(`fields.py:265-286`) fills a coordinate the spec's own `derive` rule
...
(`fields.py:212-235`), written directly in `runner.py:3343-3356`, not
...
quote sits in a shown source and carries the answer; `unstated` when the
model names the
...
`tests/test_extraction_fieldwise.py:283`); and `unbacked` when a quote is
...
keeps its first reading (`tests/test_extraction_fieldwise.py:1036`).
...
`tests/test_extraction_fieldwise.py:330`). `unstated` becomes permanent
...
`tests/test_extraction_fieldwise.py:310`). See
...
(`docpipe/extraction/pipeline.py:1219`). `exhausted` and `unstated` are the
...
(`docpipe/extraction/runner.py:3205`). The two the sweep skips are kept
