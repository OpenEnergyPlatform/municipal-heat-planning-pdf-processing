...
(`fields.py:212-235`), written directly in `runner.py:3269-3282`, not
...
(`docpipe/extraction/pipeline.py:1118`). `exhausted` and `unstated` are the
...
(`docpipe/extraction/runner.py:3133`). The two the sweep skips are kept
...
(`docpipe/extraction/runner.py:3319`). The same split holds past the
