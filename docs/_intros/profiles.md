...
`docpipe/extraction/runner.py:4006`, `docpipe/chunking/pipeline.py:346`),
...
| Reading the values out | `extraction.SPEC_PATH` | required in practice; refused with `SystemExit` otherwise (`component`, `docpipe/extraction/runner.py:4060-4063`) | `extraction_spec.json` | `extraction_spec.json` |
| Reading the values out | `extraction.SLICE` | optional (`component`, `docpipe/extraction/runner.py:4112`) | `{"quantity": None}`, gates a row on its quantity class alone | not provided; no gate |
| Reading the values out | `extraction.FRAME` | optional (`component`, `docpipe/extraction/runner.py:4132`) | `("scenario", "year")`, found once per document | not provided; nothing repeats that way |
| Reading the values out | `extraction.document_axes` | optional (`component`, `docpipe/extraction/runner.py:4123`) | not provided | this publication's AR6 scenarios and the study regions it names, out of 249 (`profiles/scenarios/regions.json`) |
| Reading the values out | `extraction.document_context` | optional (`component`, `docpipe/extraction/runner.py:4128`) | the plan's own municipality name | not provided |
| The knowledge graph | `kg.make_serializer` | required for `--serialize`; refused with `SystemExit` otherwise (`component`, `docpipe/extraction/runner.py:4039-4042`) | tuples to MHPKG Turtle | tuples to OEKG Turtle |
...
`backfill_meta`, and
`docpipe/extraction/runner.py:4039-4042,4060-4063` for a missing
