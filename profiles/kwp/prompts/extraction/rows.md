...
Deine Aufgabe in diesem Schritt ist EINE: jede Zahl des gesuchten Parameters finden, ihre Einheit nennen und die Passage zitieren, in der sie steht. Alles andere — Energieträger, Sektor, Jahr, Szenario, Gebiet, Größe — wird DANACH gefragt, für jede Angabe einzeln und mit eigenem Beleg. Du musst hier nichts davon zuordnen und sollst es auch nicht.

Du bekommst ein JSON-Objekt mit diesen Feldern:

- "parameter": die gesuchte Kennzahl — Label, Beschreibung, akzeptierte Einheiten ("units_accepted") und ein vollständiges Beispiel ("example": ein echter Quellausschnitt plus die Zahlen, die daraus zu holen sind).
...
2. "unit" und "unit_raw":
   ...
   Steht in der Quelle eine Einheit, die in "units_accepted" keine Entsprechung hat, lässt du "unit" leer und füllst nur "unit_raw". Rechne NIE um.
...
5. "status" und "need_more":
   - "complete": alles, was der gesuchte Parameter in diesen Quellen an Zahlen hergibt, steht in "tuples". Auch dann, wenn "tuples" leer ist. Das ist der Normalfall.
   - "partial": in einer der Quellen steht eine Zahl, deren EINHEIT du nicht bestimmen kannst, weil sie anderswo im Plan steht. Nur dafür. Fehlende Träger, Jahre oder Szenarien sind hier kein Grund — danach wird gar nicht gefragt.
