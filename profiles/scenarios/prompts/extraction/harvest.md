...
6. "scenario": nur bei den Feldern, die im Parameter eine Achse "scenario" haben. Diese Achse führt unter "classes" die Szenarien, die GENAU DIESE Publikation in der AR6-Datenbank dokumentiert. Trage dort den Klassennamen des Szenarios ein, auf das sich der Wert bezieht, und in "scenario_raw" den Namen, den das Dokument dafür verwendet.
   ...
   Viele Kennungen teilen sich einen Anfang: EN_NPi2020_400, EN_NPi2020_1000 und EN_NPi2020_3000 sind DREI verschiedene Läufe. Nennt das Dokument nur den gemeinsamen Anfang ("NPi", "NDC"), dann ist damit KEINE einzelne Kennung bestimmt, und du darfst dir keine aussuchen — dafür steht "Szenario-Familie". Eine Kennung setzt du, wenn der Text sie unterscheidbar macht, etwa durch das Budget, das Temperaturziel oder das Jahr im Namen.
   Die beiden Ausgänge bedeuten NICHT dasselbe, und der Unterschied ist mechanisch prüfbar: geh die Liste durch und sieh nach, wie viele Einträge mit deinem Wortlaut anfangen oder ihn enthalten.
   - MEHRERE Einträge treffen zu → "Szenario-Familie".
   - GENAU EINER trifft zu → das ist kein Ausgang, sondern die Antwort. Trag die Kennung ein.
   - KEIN EINZIGER trifft zu → "nicht in AR6". Das Dokument beschreibt ein Szenario, das seine AR6-Liste nicht führt.
   Das ist keine Feinheit. Im letzten Korpuslauf standen 17012 Antworten auf "Szenario-Familie", und bei 14656 davon fing KEIN Eintrag der Liste mit dem Wortlaut an — das waren keine Familien, das waren Szenarien außerhalb von AR6. Umgekehrt wurden 1171 Kennungen verschenkt, die genau einen Eintrag trafen.
